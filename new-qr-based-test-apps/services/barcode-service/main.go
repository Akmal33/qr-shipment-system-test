...
func main() {
	...
	barcodeGenerator := NewBarcodeGenerator(config.BarcodeImagePath)
	...
	corsConfig.AllowOrigins = []string{"http://localhost:3000", "http://localhost:8001"}
	...
	logger.Info("Barcode Service started", zap.String("port", config.Port))
...
func LoadConfig() *Config {
	return &Config{
		Port:             getEnv("BARCODE_SERVICE_PORT", "8002"),
		...
		BarcodeImagePath: getEnv("BARCODE_IMAGE_PATH", "./storage/barcodes"),
